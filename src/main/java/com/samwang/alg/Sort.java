...
public class Sort {
  ...
  private static Comparable[] insertSort(Comparable[] t) {
    for (int i = 1; i < t.length; i++) {
      ...
      for (int j = i; j > 0; j--) {
        if (Util.isLess(t[j], t[j-1])) {
          Util.exchange(t, j, j-1);
        }
      }
    }
    return t;
    ...
    while (pace > 0) {
      ...
      for (int j = 0; j < length; j += pace) {
        for (int k = j; k >= pace; k -= pace) {
          if (Util.isLess(t[k], t[k-pace])) {
            Util.exchange(t, k, k-pace);
          }
        }
      }
  ...
  private static void merge(Comparable[] t, int lo, int mi, int hi, Comparable[] buff) {
    ...
    for (int i = 0; i < len; i++) {
      ...
      if (index1 > mi && index2 <= hi) {
        ...
        if (Util.isLess(t[index1], t[index2])) {
          buff[lo + i] = t[index1++];
        } else {
          buff[lo + i] = t[index2++];
        }
  ...
  private static int makePartition(Comparable[] t, int lo, int hi, int sp) {
    ...
    int j = lo+1, k = hi;
    while (j < k) {
      while (j <= hi && Util.isLess(t[j], anchor)) j++;
      while (k >= lo && Util.isLess(anchor, t[k])) k--;
    ...
    if (j > k) {
      Util.exchange(t, lo, k);
      return k;
    } else {
      int anchorIndex = Util.isLess(t[k], t[lo]) ? k : k-1;
      Util.exchange(t, lo, anchorIndex);
      return anchorIndex;
    }
  }
